...
assert.equal(decision.reason, 'delivery-timeout');
...
assert.equal(decision.reason, 'blocked-state');
...
assert.equal(core.capacityLevel({ status:'capacity-watch', turnCount:1, charCount:1 }, {}), 'watch', 'authoritative Sentinel watch state wins');
assert.equal(core.capacityLevel({ status:'capacity-handoff', turnCount:1, charCount:1 }, {}), 'handoff', 'authoritative Sentinel handoff state wins');
assert.equal(core.capacityLevel({ status:'capacity-reached', turnCount:1, charCount:1 }, {}), 'handoff', 'provider hard-limit state wins');

...
<section data-testid="conversation-turn-2"><h4>ChatGPT said:</h4><div data-message-author-role="assistant" data-message-id="e7ca69ef-127d-4b3e-91b4-18cde0a6c386"><p>Constellation selector check.</p></div></section>
...
with sync_playwright() as p:
    ...
    result=page.evaluate('''() => {const turns=window.__brain.filter(row=>row.type==='TURN_UPSERT');const chats=window.__brain.filter(row=>row.type==='CHAT_UPSERT');return {turnCount:turns.length,roles:turns.map(row=>row.data.role),ids:[...new Set(turns.map(row=>row.data.chatId))],chatIds:[...new Set(chats.map(row=>row.data.id))],composer:document.querySelector('[role="textbox"]')?.innerText,sections:document.querySelectorAll('[data-testid^="conversation-turn-"]').length};}''')

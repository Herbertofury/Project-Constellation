...
with sync_playwright() as p:
    ...
    result=page.evaluate('''() => ({sentPrompt:window.__sentPrompt,completeCalls:window.__completeCalls,lineageCalls:window.__lineageCalls,newChatId:window.__newChatId,composerText:document.getElementById('prompt-textarea').innerText,toast:!!document.getElementById('projectConstellationBranchToast'),nativeForm:!!document.querySelector('form'),errors:[]})''')
    print(json.dumps({'result':result,'errors':errors},sort_keys=True))
    assert 'Project Constellation Safe Handoff' in result['sentPrompt'] and 'direct continuation' in result['sentPrompt']
    assert result['newChatId']=='chatgpt:continued' and result['composerText']=='' and result['nativeForm']
    assert result['completeCalls'] and result['completeCalls'][-1]['status']=='sent'
    assert any(row.get('chatId')=='chatgpt:continued' for row in result['lineageCalls'])
    assert result['toast'] and not errors
    browser.close()

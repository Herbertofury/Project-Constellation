...
chat3={'id':'chatgpt:minecraft-test','providerId':'chatgpt','title':'Minecraft compatibility test','url':'https://chatgpt.com/c/minecraft-test','workspaceProjectId':'local:project:minecraft','workspaceProjectName':'Minecraft Mods','workspaceGroupId':'local:group:modding','status':'stalled','tags':['minecraft','test'],'pinned':False,'favorite':False,'lastExcerpt':'Waiting on runtime proof','updatedAt':1759500000000}
org={'groups':[{'id':'local:group:modding','name':'Modding','icon':'M','color':'#6f8cff','parentId':'','pinned':True,'updatedAt':1760000000000}], 'projects':[{'id':'local:project:minecraft','sourceType':'workspace','name':'Minecraft Mods','description':'Forge and Fabric repairs','groupId':'local:group:modding','icon':'C','color':'#7d92ff','pinned':True,'chatCount':2,'fileCount':1,'activeCount':1,'attentionCount':1,'completedCount':0,'archivedCount':0,'updatedAt':1760000000000}], 'providerProjects':[{'id':'chatgpt:source:minecraft','providerId':'chatgpt','name':'Minecraft source','sourceType':'provider','updatedAt':1760000000000}], 'smartCollections':[{'id':'local:smart:forge','name':'Forge crashes','query':'forge crash','groupId':'local:group:modding','icon':'S','pinned':True,'updatedAt':1760000000000}], 'tags':[{'name':'minecraft','count':1},{'name':'forge','count':1},{'name':'skyrim','count':1}], 'pinnedChats':[chat1], 'favoriteChats':[chat2], 'unassignedCount':1}
summary={'counts':{'providers':2,'projects':2,'chats':3,'turns':22,'files':1},'recentChats':[chat1,chat2,chat3],'recentFiles':[],'recentProjects':org['providerProjects'],'recentEvents':[],'attention':[],'live':[],'topics':[{'term':'minecraft','count':4}],'catalog':None,'fullCapture':None,'discovery':{'browserHistoryGranted':True,'mode':'zero-tab-default','hiddenTabs':False,'manualFullCapture':True},'organization':org,'sync':{'drive':{'lastStatus':'verified','lastSyncAt':1760000000000,'oauthProvisioned':True},'github':{'configured':False,'lastSyncAt':0}}}
providers=[{'id':'chatgpt','name':'ChatGPT','catalog':{}},{'id':'grok','name':'Grok','catalog':{}}]
project_brains={'local:project:minecraft':{'compiledBrain':{'projectId':'local:project:minecraft','projectName':'Minecraft Mods','coverage':{'knowledgeItems':4,'sourceChats':2,'files':1,'ignored':0,'superseded':0,'pinned':1},'workingSet':[{'id':'decision-1','kind':'decision','title':'Keep Forge runtime verification','text':'Keep Forge runtime verification','confidence':0.94,'pinned':True,'source':{'chatTitle':'Minecraft mod repair','chatUrl':'https://chatgpt.com/c/minecraft'}}],'activeChats':[{'id':'chatgpt:minecraft','title':'Minecraft mod repair','url':'https://chatgpt.com/c/minecraft','bucket':'active','updatedAt':1760000000000},{'id':'chatgpt:minecraft-test','title':'Minecraft compatibility test','url':'https://chatgpt.com/c/minecraft-test','bucket':'attention','updatedAt':1759500000000}],'history':[]}}}
mock=f"""
(() => {{
 const opened=[]; const messages=[]; const org={json.dumps(org)}; let chats={json.dumps([chat1,chat2,chat3])}; const brainByProject={json.dumps(project_brains)};
 ...
 globalThis.chrome={{runtime:{{id:'pc-test',sendMessage:async(m)=>{{messages.push(m);
   ...
   if(m.type==='PC_PROJECT_BRAIN_GET')return {{ok:true,continuity:brainByProject[m.projectId]||null}};
   if(m.type==='PC_PROJECT_MEMORY_GATE'){{window.__memoryGate={{itemId:m.itemId,action:m.action}};return {{ok:true,result:{{}}}};}}
...
with sync_playwright() as p:
    ...
    assert 'Active' in page.locator('#orgChatList').inner_text() and 'Needs attention' in page.locator('#orgChatList').inner_text()
    assert page.locator('#orgChatList .org-project-cluster').count()>=2
    page.locator('#orgProjectGrid [data-org-project="local:project:minecraft"]').click(); page.wait_for_timeout(90)
    assert page.locator('#orgProjectBrain').is_visible(); assert 'Keep Forge runtime verification' in page.locator('#orgProjectBrain').inner_text(); assert 'Minecraft compatibility test' in page.locator('#orgProjectBrain').inner_text()
    page.locator('#orgProjectBrain [data-memory-action="unpin"]').click(); page.wait_for_timeout(40); assert page.evaluate('()=>window.__memoryGate')=={'itemId':'decision-1','action':'unpin'}
    page.locator('#orgGroupMode').select_option('state'); page.wait_for_timeout(30); assert page.locator('#orgChatList .org-project-cluster').count()==0
    page.locator('#orgGroupMode').select_option('project-state'); page.wait_for_timeout(30)
    page.locator('[data-org-mode="all"]').click(); page.wait_for_timeout(50)

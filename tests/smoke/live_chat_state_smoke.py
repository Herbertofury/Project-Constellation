...
with sync_playwright() as p:
    ...
    page.set_content(html,wait_until='load');page.evaluate(mock);page.add_script_tag(content=core);page.add_script_tag(content=brain);page.add_script_tag(content=health);page.add_script_tag(content=content);page.wait_for_timeout(350)
    active=page.evaluate("__sendToContent({type:'PC_GET_LIVE_CHAT_STATE'})")
    page.evaluate("""() => {document.getElementById('tool').textContent='Searched Google Drive for Creeperella 1.4.16 files';const b=document.createElement('button');b.setAttribute('aria-label','Copy');b.textContent='Copy';document.getElementById('assistant-turn').appendChild(b);}""")

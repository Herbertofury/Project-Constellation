(() => {
  ...
  const ids = ['enabled','responsiveScrolling','adaptiveMotionRelief','pressure','status','longTasks','maxTask','provider','chatState','resetMetrics','openHome','openConstellation'];
  ...
  function renderStatus(status){
    ...
    els.provider.textContent=status.provider?.name||'AI'; els.chatState.textContent=status.chat?.status||'idle'; els.longTasks.textContent=status.metrics?.totalLongTasks||0; els.maxTask.textContent=`${status.metrics?.maxLongTaskMs||0} ms`;
    els.status.textContent='Performance protection and continuity capture are active on this tab.';
  }
  ...
  els.openConstellation.addEventListener('click',async()=>{ const [tab]=await chrome.tabs.query({active:true,currentWindow:true}); if(tab?.windowId){ await chrome.sidePanel.open({windowId:tab.windowId}); window.close(); } });

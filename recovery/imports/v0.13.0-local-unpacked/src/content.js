(() => {
  ...
  function detectToolEvidence() {
    ...
    const selector = '[data-testid*="tool" i],[aria-label*="tool" i],[class*="tool" i],[data-state],[aria-busy],button,[role="button"],[aria-live]';
    const nodes = [...rootNode.querySelectorAll(selector)].slice(-360);
    const entries = [];
    for (const node of nodes) {
  ...
  function healthHudCss() {
    return `
      :host{all:initial;position:fixed;z-index:2147483000;font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:#f5f7fb;--pc-accent:#7f92ff;--pc-level:#63d6a7;--pc-bg:rgba(13,15,20,.94);--pc-line:rgba(255,255,255,.12);--pc-muted:#9ca4b3;--pc-shadow:0 18px 60px rgba(0,0,0,.46);pointer-events:none}
      ...
      .hud{pointer-events:auto;width:350px;box-sizing:border-box;border:1px solid color-mix(in srgb,var(--pc-level) 32%,var(--pc-line));border-radius:16px;background:linear-gradient(145deg,color-mix(in srgb,var(--pc-bg) 92%,var(--pc-level) 8%),var(--pc-bg));box-shadow:var(--pc-shadow);backdrop-filter:blur(18px) saturate(1.12);overflow:hidden;transition:width .18s ease,transform .18s ease,border-color .18s ease}
  ...
  function scheduleLiveHealthPulse(delay) {
    ...
    const nextDelay = delay ?? (document.hidden ? 15000 : active ? liveHealthSettings.pollActiveMs : liveHealthSettings.pollIdleMs);
    liveHealthTimer = setTimeout(() => { liveHealthTimer = 0; updateLiveHealth().finally(() => scheduleLiveHealthPulse()); }, Math.max(750, Number(nextDelay || 2500)));
  }
  ...
  document.addEventListener('visibilitychange', () => {
    ...
    scheduleLiveHealthPulse(document.hidden ? 15000 : 400);
  }, { passive: true });

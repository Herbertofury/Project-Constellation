(() => {
  ...
  function renderAttention(){
    ...
    $('approvalRecoveryStatus').textContent=running
      ? `Background recovery · ${fmt(scanned)} / ${fmt(total)} scanned · ${fmt(state.recovered)} recovered · ${fmt(state.alwaysAllowed)} persistent · ${fmt(state.refreshed||0)} browser-refresh fixed · ${fmt(state.failed)} failed${state.currentChatId?` · ${state.currentChatId}`:''}`
  ...
  function renderSources(){
    ...
    $('catalogHomeStatus').textContent=catalog?`${catalog.status} · ${fmt(catalog.discovered)} discovered · ${fmt(catalog.captured)} fetched · ${fmt(catalog.turnsCaptured)} turns · ${fmt(catalog.metadataOnly)} metadata-only · ${fmt(catalog.skippedFresh||0)} fresh chats skipped · ${fmt(catalog.notModified||0)} unchanged`:'Ready. Zero-tab catalog opens no crawler tabs; Approval Autopilot has its own single recovery tab when enabled.';
  ...
  $('connectGithubAccount').addEventListener('click',async()=>{try{
    ...
    await chrome.tabs.create({url:githubVerificationUrl,active:true});if(githubPollTimer)clearTimeout(githubPollTimer);githubPollTimer=setTimeout(()=>pollGithubOAuth().catch((e)=>toast(e.message)),Math.max(1000,Number(result.intervalMs||5000)));
  ...
  async function saveApprovalAutopilot(patch){const current=home?.approvalAutopilot||{};const r=await call({type:'PC_BRAIN_SETTINGS_SET',settings:{approvalAutopilot:{...current,...patch}}});if(!r?.ok)throw new Error(r?.error||'Could not save Approval Autopilot settings.');await loadHome();return r;}
  async function saveLiveHealth(patch){const current=home?.liveHealth||{};const r=await call({type:'PC_BRAIN_SETTINGS_SET',settings:{liveHealth:{...current,...patch}}});if(!r?.ok)throw new Error(r?.error||'Could not save Live Chat Health settings.');await loadHome();return r;}
  async function updateLiveHealthSetting(patch){try{$('liveHealthSettingsStatus').textContent='Saving Live Chat Health settings…';await saveLiveHealth(patch);$('liveHealthSettingsStatus').textContent='Saved · open AI tabs update without a reload.';}catch(e){$('liveHealthSettingsStatus').textContent=e.message;toast(e.message);}}
  ...
  $('approvalRiskAcknowledged').addEventListener('change',async()=>{try{await saveApprovalAutopilot({acknowledged:$('approvalRiskAcknowledged').checked,enabled:$('approvalRiskAcknowledged').checked?Boolean(home?.approvalAutopilot?.enabled):false});}catch(e){toast(e.message);}});
  ...
  $('approvalAutopilotEnabled').addEventListener('change',async()=>{try{if($('approvalAutopilotEnabled').checked&&!$('approvalRiskAcknowledged').checked){$('approvalAutopilotEnabled').checked=false;toast('Acknowledge the Always Allow behavior first.');return;}await saveApprovalAutopilot({enabled:$('approvalAutopilotEnabled').checked,acknowledged:$('approvalRiskAcknowledged').checked});if($('approvalAutopilotEnabled').checked){$('approvalRecoveryStatus').textContent='Autopilot enabled · running one full cleanup of known ChatGPT chats…';await call({type:'PC_APPROVAL_RECOVERY_START',mode:'all-known'});await loadHome();}}catch(e){toast(e.message);}});
  $('approvalFallbackAllowOnce').addEventListener('change',async()=>{try{await saveApprovalAutopilot({fallbackAllowOnce:$('approvalFallbackAllowOnce').checked});}catch(e){toast(e.message);}});
  $('approvalAutoRecoverPaused').addEventListener('change',async()=>{try{await saveApprovalAutopilot({autoRecoverPaused:$('approvalAutoRecoverPaused').checked});}catch(e){toast(e.message);}});
  $('fixDetectedChats').addEventListener('click',async()=>{try{$('approvalRecoveryStatus').textContent='Building blocked/stale recovery queue…';const r=await call({type:'PC_APPROVAL_RECOVERY_START',mode:'attention'});if(!r?.ok)throw new Error(r?.error||'Recovery could not start.');await loadHome();}catch(e){$('approvalRecoveryStatus').textContent=e.message;}});
  $('fixAllBlockedChats').addEventListener('click',async()=>{try{$('approvalRecoveryStatus').textContent='Deep-scanning every known ChatGPT chat in one hidden recovery window…';const r=await call({type:'PC_APPROVAL_RECOVERY_START',mode:'all-known'});if(!r?.ok)throw new Error(r?.error||'Fix All could not start.');await loadHome();}catch(e){$('approvalRecoveryStatus').textContent=e.message;}});
  $('stopApprovalRecovery').addEventListener('click',async()=>{const r=await call({type:'PC_APPROVAL_RECOVERY_STOP'});if(!r?.ok)toast(r?.error||'Could not stop recovery.');await loadHome();});

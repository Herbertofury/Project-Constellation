(() => {
  ...
  $('panelFixDetected').addEventListener('click',async()=>{$('panelApprovalStatus').textContent='Building detected blocked/stale queue…';const r=await call({type:'PC_APPROVAL_RECOVERY_START',mode:'attention'});if(!r?.ok)$('panelApprovalStatus').textContent=r?.error||'Recovery failed to start.';await load();});
  $('panelFixAll').addEventListener('click',async()=>{$('panelApprovalStatus').textContent='Deep-scanning all known ChatGPT chats…';const r=await call({type:'PC_APPROVAL_RECOVERY_START',mode:'all-known'});if(!r?.ok)$('panelApprovalStatus').textContent=r?.error||'Fix All failed to start.';await load();});

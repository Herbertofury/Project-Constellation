...
const LIVE_HEALTH_CONTEXT_TTL_MS = 3500;
...
async function githubTokenValue() {
  return String((await chrome.storage.local.get(GITHUB_SECRET_KEY))[GITHUB_SECRET_KEY] || '');
}
...
async function githubOAuthPoll() {
  ...
  await chrome.storage.local.set({ [GITHUB_SECRET_KEY]: data.access_token });
  if (data.refresh_token) await chrome.storage.local.set({ [GITHUB_REFRESH_KEY]: data.refresh_token });
  await area.remove(GITHUB_OAUTH_PENDING_KEY);
...
async function githubOAuthDisconnect() {
  await chrome.storage.local.remove([GITHUB_SECRET_KEY, GITHUB_REFRESH_KEY]);
...
async function driveConnectionStatus({ verify = false } = {}) {
  ...
  if (verify && googleOAuthProvisioned()) {
    try {
      ...
      else if (response.status === 401) connected = false;
    } catch (e) { if (!/interaction|OAuth|sign/i.test(String(e?.message || e))) error = String(e?.message || e); }
  }
...
if (chrome.tabs?.onRemoved) chrome.tabs.onRemoved.addListener((tabId) => liveNetworkByTab.delete(Number(tabId)));

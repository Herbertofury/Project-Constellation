...
function isChatGptUrl(value) {
  try { return /^(chatgpt\.com|chat\.openai\.com)$/i.test(new URL(String(value || '')).hostname); }
...
function mutateState(fn) {
  const task = stateQueue.catch(() => {}).then(async () => fn(await readState()));
...
async function notify(id, title, message) {
  try {
    await chrome.notifications.create(safeId(id), {
      type:'basic', iconUrl:chrome.runtime.getURL('assets/constellation-field.svg'),
      title:String(title || 'Project Constellation').slice(0,120), message:String(message || '').slice(0,420), priority:2
    });
  } catch (_) {}
}
function recoveryConfig(cfg = {}) {
  const row = cfg.refreshRecovery || {};
  return core.normalizeConfig({
    staleRunningMs:Number(row.staleChatRescueMs || 2 * 60 * 60 * 1000),
    deadRecoveryMs:Number(row.deadRecoveryMs || 4 * 60 * 1000),
    failureRecoveryDelayMs:Number(row.failureRecoveryDelayMs || 3500),
    recoveryCooldownMs:Number(row.cooldownMs || 10 * 60 * 1000),
    maxRecoveriesPerChat:Number(row.maxRefreshesPerChat || 4)
  });
}

async function beginRecovery(tab, snapshot, row, decision, state) {
  const tabId = Number(tab?.id || 0);
  if (!tabId || !isChatGptUrl(snapshot.url)) return false;
  const now = Date.now();
  const recoveryId = `${snapshot.chatId}:${now}:${Math.random().toString(36).slice(2,8)}`;
  const pending = {
    recoveryId,
    prompt:core.buildContinuationPrompt({ reason:decision.reason, projectName:snapshot.projectName, title:snapshot.title }),
    reason:decision.reason, status:'reload-issued', issuedAt:now, tabId, resumeAttempts:0
  };
  const next = { ...row, chatId:snapshot.chatId, url:snapshot.url, title:snapshot.title,
    projectName:snapshot.projectName || row.projectName || '', lastRecoveryAt:now,
    recoveryCount:Number(row.recoveryCount || 0) + 1, pending, updatedAt:now };
  state.chats[snapshot.chatId] = next;
  ...
  try {
    ...
    next.pending = { ...pending, status:'reload-failed', error:String(error?.message || error).slice(0,240) };
...
async function handleSnapshot(port, snapshot = {}) {
  ...
  if (!snapshot.chatId || !tab?.id || !isChatGptUrl(snapshot.url || tab.url || '')) return;
  const cfg = await settings();
  await mutateState(async (state) => {
    const now = Date.now();
    const previous = state.chats[snapshot.chatId] || { chatId:snapshot.chatId, recoveryCount:0 };
    const signatureChanged = Boolean(snapshot.signature && snapshot.signature !== previous.signature);
    ...
    const row = {
      ...previous, chatId:snapshot.chatId, url:snapshot.url, title:snapshot.title,
      projectId:snapshot.projectId || previous.projectId || '', projectName:snapshot.projectName || previous.projectName || '',
      signature:snapshot.signature || previous.signature || '', status:snapshot.status || previous.status || 'idle',
      failureKind:snapshot.failureKind || '', failureDetectedAt, lastProgressAt, lastObservedAt:now, wasRunning,
      unresolvedUser:Boolean(snapshot.unresolvedUser), hidden:Boolean(snapshot.hidden),
      turnCount:Number(snapshot.turnCount || 0), charCount:Number(snapshot.charCount || 0), updatedAt:now
    };
    ...
    if (capacity !== 'clear' && capacity !== row.lastCapacityNotice) {
      ...
      await notify(`pc-capacity-${snapshot.chatId}-${capacity}`,
        capacity === 'handoff' ? 'Project Constellation: branch now' : 'Project Constellation: branch soon',
        capacity === 'handoff'
          ? `${snapshot.title || 'This chat'} reached the proactive handoff boundary. Secure a continuation before the provider cuts it off.`
          : `${snapshot.title || 'This chat'} is getting large. Constellation is warning early while there is still runway.`);
    } else if (capacity === 'clear' && row.lastCapacityNotice) row.lastCapacityNotice = '';
    state.chats[snapshot.chatId] = row;
    state.lastTickAt = now;
    await writeState(state);

    const decision = core.shouldRecover({
      enabled:cfg.refreshRecovery?.enabled === true, chatId:snapshot.chatId, now,
      status:row.status, failureKind:row.failureKind, failureDetectedAt:row.failureDetectedAt,
      lastProgressAt:row.lastProgressAt, lastRecoveryAt:row.lastRecoveryAt, recoveryCount:row.recoveryCount,
      wasRunning:row.wasRunning, unresolvedUser:row.unresolvedUser, config:recoveryConfig(cfg)
    });
    if (decision.recover && !row.pending) await beginRecovery(tab, snapshot, row, decision, state);
  });
}
...
async function handleResumeResult(message = {}) {
  ...
  await mutateState(async (state) => {
    ...
    if (status === 'sent') {
      row.pending = null; row.wasRunning = true; row.unresolvedUser = false; row.lastProgressAt = Date.now();
      row.lastResumeStatus = 'sent'; row.lastResumeAt = Date.now();
    } else {
      row.pending = { ...row.pending, status, resultAt:Date.now() }; row.lastResumeStatus = status;
      if (status === 'composer-busy') await notify(`pc-resume-busy-${message.chatId}`, 'Project Constellation preserved your draft', 'The rescued chat already has text in the composer, so Constellation did not overwrite it.');
    }
    row.updatedAt = Date.now(); state.chats[message.chatId] = row; return writeState(state);
  });
}
async function maybeSendResume(port, chatId) {
  if (!chatId) return;
  const state = await readState(); const row = state.chats[chatId]; const pending = row?.pending;
  if (!pending || !['reload-issued','failed','prefilled'].includes(String(pending.status || '')) || Number(pending.resumeAttempts || 0) >= 3) return;
  pending.resumeAttempts = Number(pending.resumeAttempts || 0) + 1; pending.lastResumeAttemptAt = Date.now();
  row.pending = pending; state.chats[chatId] = row; await writeState(state);
...
function openDb() {
  return new Promise((resolve,reject) => {
    const request = indexedDB.open(DB_NAME); request.onsuccess = () => resolve(request.result); request.onerror = () => reject(request.error);
  });
}
...
async function upsertProviderProject(project = {}) {
  ...
  try {
    ...
    await new Promise((resolve,reject) => {
      const names = db.objectStoreNames.contains('searchDocs') ? ['projects','searchDocs'] : ['projects'];
      const tx = db.transaction(names,'readwrite'); const store = tx.objectStore('projects'); const get = store.get(project.id);
      get.onsuccess = () => {
        const merged = { ...(get.result || {}), ...project, sourceType:'provider', providerId:'chatgpt', updatedAt:Number(project.updatedAt || Date.now()) };
        store.put(merged);
        if (names.includes('searchDocs')) tx.objectStore('searchDocs').put({
          id:`project:${merged.id}`, entityType:'project', entityId:merged.id, providerId:'chatgpt', title:merged.name,
          text:`${merged.name} ${merged.url || ''}`, url:merged.url || '', terms:projectTerms(merged), updatedAt:merged.updatedAt
        });
      };
      get.onerror = () => reject(get.error); tx.oncomplete = resolve; tx.onerror = () => reject(tx.error);
    });
  } finally { db.close(); }
}
...
async function bootstrapTab(tab) {
  if (!tab?.id || !isChatGptUrl(tab.url || '') || tab.discarded) return false;
  try {
...
async function superviseOpenTabs() {
  ...
  await mutateState(async (state) => {
    for (const tab of tabs) {
      const chatId = core.chatIdFromUrl(tab.url || ''); const row = chatId ? state.chats[chatId] : null;
      ...
      const decision = core.shouldRecover({
        enabled:cfg.refreshRecovery?.enabled === true, chatId, now, status:row.status, failureKind:row.failureKind,
        failureDetectedAt:row.failureDetectedAt, lastProgressAt:row.lastProgressAt, lastRecoveryAt:row.lastRecoveryAt,
        recoveryCount:row.recoveryCount, wasRunning:row.wasRunning, unresolvedUser:row.unresolvedUser, config:recoveryConfig(cfg)
      });
      if (!decision.recover) continue;
      await beginRecovery(tab, { chatId, url:tab.url || row.url || '', title:tab.title || row.title || 'ChatGPT', projectName:row.projectName || '' }, row, decision, state);
    }
    state.lastTickAt = now; return writeState(state);
  });
...
chrome.runtime.onConnect.addListener((port) => {
  ...
  const tabId = Number(port.sender.tab.id); ports.set(tabId,port);

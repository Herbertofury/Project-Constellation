...
for (const marker of ['structuredTurnLinks','structuredCodeBlocks']) if (!content.includes(marker)) throw new Error(`Structured knowledge capture missing: ${marker}`);
...
for (const marker of ['deriveHealth','stale-page','tool-running','tool-stalled','tool-dead','request-stalled','dead','refresh-required','old-project-version']) if (!healthCore.includes(marker)) throw new Error(`Live Chat Health core missing: ${marker}`);
for (const marker of ['projectConstellationHealthHud','PC_LIVE_HEALTH_CONTEXT','renderLiveHealthHud','liveHealthSettings','toolPhaseFromLabel','liveHealthToolSteps','Tool pulse']) if (!content.includes(marker)) throw new Error(`Live Chat Health content workflow missing: ${marker}`);
...
for (const marker of ['PC_LIVE_HEALTH_CONTEXT','liveNetworkByTab','oldestPendingAt','chrome.webRequest.onBeforeRequest','latestTurnsForChat']) if (!background.includes(marker)) throw new Error(`Live Chat Health backend missing: ${marker}`);
